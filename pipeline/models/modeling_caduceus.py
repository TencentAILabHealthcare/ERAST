...
class CaduceusForSequenceClassification(CaduceusPreTrainedModel):
    def __init__(
        ...
        factory_kwargs = {"device": device, "dtype": dtype}
        self.num_labels = kwargs.get("num_labels", num_labels)
        self.caduceus = Caduceus(config, **factory_kwargs, **kwargs)
    ...
    def inference(self,input_ids):
        ...
        pred_label=torch.argmax(logits, 1).detach().cpu().numpy()
        return pooled_hidden_states.detach().cpu().numpy(), pred_label
